...
export function LoopOptionsSection({
  ...
  const visible = loopOptions.length > 1;
  return (
    <div className={cn(
      "grid transition-all duration-300 ease-in-out border-t border-border",
      visible ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0 border-t-0"
    )}>
      {/* overflow-hidden on inner div is required for grid-rows animation */}
